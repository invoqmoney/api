...
`t` คือ Unix timestamp หน่วยวินาที ส่วน `v1` คือ HMAC-SHA256 ในรูปเลขฐานสิบหกตัวพิมพ์เล็กของ `<t>.<raw_body>` โดยใช้คีย์ webhook ของโหมดนั้น ให้ตรวจสอบกับ **body ดิบ** ก่อนแปลงเป็น JSON — การ serialize JSON ใหม่อาจทำให้ไบต์เปลี่ยนและลายเซ็นใช้ไม่ได้ และให้ปฏิเสธ timestamp ที่อยู่นอกช่วงที่คุณยอมรับได้ SDK ทางการทุกตัวมีฟังก์ชันนี้มาให้แล้ว โดยตั้งชื่อตามภาษาของตัวเอง (`verifyWebhook`, `verify_webhook`, `VerifyWebhook`)
